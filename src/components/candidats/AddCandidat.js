...
import { createCandidat, resetMessage } from "../../features/candidatsSlice";
import moment from "moment";
const AddCandidat = () => {
  const [error, setError] = useState("");
  const { message } = useSelector((state) => state.candidats);
  ...
  const onSubmit = (e) => {
    ...
    dispatch(createCandidat(newCandidat));
  };
  return (
